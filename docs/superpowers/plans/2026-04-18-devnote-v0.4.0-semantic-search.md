...
def test_search_returns_top_k_above_threshold():
    ...
    v_related[0] = 0.99
    v_related /= np.linalg.norm(v_related)
...
git commit -m "test(v0.4.0): add 11 pytest tests for python worker"
```
...
async function runTests() {
  ...
  const bridge = new PythonBridge(
    'python3',  // or 'python' on Windows — assumes Python is on PATH
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const sidebarProvider = new SidebarProvider(
    ...
    draftStore,
    memoryStore,
    () => searchService,  // getter — may return null if key not set yet
  );
...
```typescript
  private async handleSaveSetup(geminiKey: string, notionToken: string, notionDbId: string): Promise<void> {
    ...
    const searchService = this.getSearchService();
    if (searchService) {
      searchService.updateApiKey(geminiKey);
    }
