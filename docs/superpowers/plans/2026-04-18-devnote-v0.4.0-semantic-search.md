...
> **Post-fix note (2026-04-18):** During E2E testing, the Gemini API returned 404 for all older embedding model names on both v1beta and v1 endpoints for AI Studio keys. Final shipped configuration: google-genai SDK + gemini-embedding-001 model + EMBEDDING_DIM = 3072. See commit history and CHANGELOG for the migration story.
...
**Goal:** Add semantic search over all past notes via Gemini `embedding-001` embeddings, executed by a lazy-spawned Python subprocess. TypeScript orchestrates, Python owns the AI/ML layer.
...
**Tech Stack:** TypeScript 5, VS Code Extension API (child_process), Python 3.10+, `google-genai>=0.8.0`, `numpy==2.1.3`, pytest, sql.js (already installed), pinned exact versions.
...
def embed_text(text: str, task_type: str = "RETRIEVAL_DOCUMENT") -> list[float]:
    """Embed a single piece of text via Gemini. Returns 3072 floats."""
...
def search(query_vec: np.ndarray, k: int = 5, threshold: float = 0.35) -> list[tuple[str, float]]:
    ...
    # Dot product — embeddings are pre-normalized by gemini-embedding-001
...
def test_embed_text_returns_3072_floats():
    ...
    with patch("worker.client.models.embed_content") as mock:
        mock.return_value = type('obj', (object,), {'embedding': fake_embedding})()
...
def test_batch_embed_returns_multiple_vectors():
    ...
    with patch("worker.client.models.embed_content") as mock:
        mock.return_value = type('obj', (object,), {'embedding': fake_batch})()
...
```javascript
  ...
  test('UPDATE embedding stores Float32Array as BLOB', () => {
    ...
    db.run(
      `UPDATE notes SET embedding = ?, embedding_model = ? WHERE id = ?`,
      [Buffer.from(fakeEmbedding.buffer), 'embedding-001', 'uuid-emb']
    );
    ...
    assert.strictEqual(model, 'embedding-001');
    db.close();
  });
  ...
  test('UPDATE nulls embedding and embedding_model for all rows', () => {
    ...
    db.run(
      `INSERT INTO notes (id, title, branch_name, content_json, content_markdown, embedding, embedding_model, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      ['a', 'a', 'main', SAMPLE_NOTE_JSON, SAMPLE_MARKDOWN, Buffer.from(vec.buffer), 'embedding-001', 1000]
    );
