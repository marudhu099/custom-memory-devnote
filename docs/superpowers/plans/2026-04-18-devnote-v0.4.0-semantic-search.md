...
**Goal:** Add semantic search over all past notes via Gemini `text-embedding-004` embeddings, executed by a lazy-spawned Python subprocess. TypeScript orchestrates, Python owns the AI/ML layer.
...
```javascript
  ...
  test('UPDATE embedding stores Float32Array as BLOB', () => {
    ...
    db.run(
      `UPDATE notes SET embedding = ?, embedding_model = ? WHERE id = ?`,
      [Buffer.from(fakeEmbedding.buffer), 'text-embedding-004', 'uuid-emb']
    );
    ...
    assert.strictEqual(model, 'text-embedding-004');
    db.close();
  });
  ...
  test('UPDATE nulls embedding and embedding_model for all rows', () => {
    ...
    db.run(
      `INSERT INTO notes (id, title, branch_name, content_json, content_markdown, embedding, embedding_model, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      ['a', 'a', 'main', SAMPLE_NOTE_JSON, SAMPLE_MARKDOWN, Buffer.from(vec.buffer), 'text-embedding-004', 1000]
    );
