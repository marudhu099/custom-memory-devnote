...
def test_vector_store_warm_load():
    ...
    assert worker.store.matrix.shape == (2, 768)
...
def test_search_returns_top_k_above_threshold():
    ...
    v_match = np.zeros(768, dtype=np.float32)
    v_match[0] = 1.0
    v_related = np.zeros(768, dtype=np.float32)
    v_related[0] = 0.9
    v_related[1] = np.sqrt(1 - 0.81)  # unit vector tilted off axis 0 → dot(query)=0.9
    v_weak = np.zeros(768, dtype=np.float32)
    v_weak[0] = 0.3
...
def test_search_returns_empty_when_all_below_threshold():
    v_unrelated = np.zeros(768, dtype=np.float32)
    v_unrelated[100] = 1.0
